...
insert_queries = [
    ...
    """
    INSERT OR IGNORE INTO Sitios (
        id_no, nome, descricao, data_inscricao, area_hectares, rev_bis, categoria)
    SELECT DISTINCT
        t.id_no, t.name_en, t.short_description_en, t.date_inscribed, t.area_hectares, t.rev_bis, c.categoria_short
    FROM temp_table t
    JOIN Categorias c ON t.category_short = c.categoria_short;
    """,

    """
    INSERT OR IGNORE INTO Criterios (
        id_criterio, descricao, categoria)
    SELECT DISTINCT
        t.criterio, t.criterio_desc, c.categoria_short
    FROM temp_table t
    JOIN Categorias c ON t.category_crit_short = c.categoria_short; 
    """,
    ...
    """
    DELETE FROM Datas_Secundarias
    WHERE data IS NULL
    """, 
    
    """
    INSERT OR IGNORE INTO Sitios_Numeros (
        sitio, unique_number)
    SELECT DISTINCT
    ...
    """
    INSERT OR IGNORE INTO Localizacoes (
    ...
    INSERT OR IGNORE INTO Periodos_Perigo (
        sitio, id_perigo, data_inicio, data_fim)
    SELECT DISTINCT
        s.id_no, t.id_perigo, t.perigo_inicio, t.perigo_fim
    FROM temp_table t
    JOIN Sitios s ON t.id_no = s.id_no;
    """,

    """
    DELETE FROM Periodos_Perigo
    WHERE data_inicio IS NULL
    """,

    """
    INSERT OR IGNORE INTO Paises_Codigos (
        pais, udnp_code)
    SELECT DISTINCT
    ...
    """
    INSERT OR IGNORE INTO Justificacoes (
    ...
    """
    INSERT OR IGNORE INTO Sitio_Pais (
        sitio, pais)
    SELECT DISTINCT
        s.id_no, p.iso_code
    FROM temp_table t
